...
@Component
class JwtUtils {
    @Value("\${kaz.app.jwtSecret}")
    private var jwtSecret: String? = null

    @Value("\${kaz.app.jwtExpirationMs}")
    var jwtExpirationMs = 0L

    @Value("\${kaz.app.jwtRefreshExpirationMs}")
    var refreshTokenDurationMs = 0L

    fun generateTokenFromUsername(username: String?): String? = Jwts.builder().setSubject(username).setIssuedAt(Date())
